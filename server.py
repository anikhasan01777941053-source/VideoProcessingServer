...
"""
    ...
    if audio_count > 1:
        ...
        subprocess.run(f'ffmpeg -y -i "{local_file}" -map 0:v -map 0:a -vf scale=1920:1080 -c:v libx264 -c:a copy {mkv_dir}/{title}_1080p.mkv', shell=True)
        subprocess.run(f'ffmpeg -y -i "{local_file}" -map 0:v -map 0:a -vf scale=1280:720 -c:v libx264 -c:a copy {mkv_dir}/{title}_720p.mkv', shell=True)
        subprocess.run(f'ffmpeg -y -i "{local_file}" -map 0:v -map 0:a -vf scale=854:480 -c:v libx264 -c:a copy {mkv_dir}/{title}_480p.mkv', shell=True)
        subprocess.run(f'ffmpeg -y -i "{local_file}" -map 0:v -map 0:a -vf scale=640:360 -c:v libx264 -c:a copy {mkv_dir}/{title}_360p.mkv', shell=True)
    else:
        mp4_dir = os.path.join(title_dir, "mp4")
        os.makedirs(mp4_dir, exist_ok=True)
        subprocess.run(f'ffmpeg -y -i "{local_file}" -map 0:v:0 -map 0:a:0 -vf scale=1920:1080 -c:v libx264 -c:a aac {mp4_dir}/{title}_1080p.mp4', shell=True)
        subprocess.run(f'ffmpeg -y -i "{local_file}" -map 0:v:0 -map 0:a:0 -vf scale=1280:720 -c:v libx264 -c:a aac {mp4_dir}/{title}_720p.mp4', shell=True)
        subprocess.run(f'ffmpeg -y -i "{local_file}" -map 0:v:0 -map 0:a:0 -vf scale=854:480 -c:v libx264 -c:a aac {mp4_dir}/{title}_480p.mp4', shell=True)
        subprocess.run(f'ffmpeg -y -i "{local_file}" -map 0:v:0 -map 0:a:0 -vf scale=640:360 -c:v libx264 -c:a aac {mp4_dir}/{title}_360p.mp4', shell=True)
